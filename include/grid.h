...
struct Grid {
    ...
    Grid (std::size_t Nx = 0, std::size_t Ny = 0, T value = T()) : data(Nx, std::vector<T>(Ny, value)) {}
    T& operator()(int i, int j) { return data[i][j]; }
    std::size_t sizeX() const { return data.size(); }
    std::size_t sizeY() const { return data.empty() ? 0 : data[0].size(); }
    std::size_t sizeZ() const { return 1; } // number of components, defined for T = vector below
    void swap(Grid<T>& other) { data.swap(other.data); } // swap content
    std::string to_vtk(std::string name); // convert to VTK format
};

template<>
std::size_t Grid<std::vector<double>>::sizeZ() const {
    return data[0][0].size();
}

template<>
std::size_t Grid<std::vector<Point>>::sizeZ() const {
    return data[0][0].size();
}

template<typename T> // for scalar fields
...
std::string Grid<std::vector<double>>::to_vtk(std::string name) {
    ...
    for (std::size_t k = 0; k < sizeZ(); k++) {
        xml << "                <DataArray type=\"Float64\" Name=\"" << name << k+1
            << "\" format=\"ascii\">" << std::endl;
        for (std::size_t i = 0; i < sizeX(); i++) {
            for (std::size_t j = 0; j < sizeY(); j++) {
                xml << data[i][j][k] << " ";
...
template<> // for Point (2D vector) fields
